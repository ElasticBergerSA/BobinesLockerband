...
const btnAccesEdicio =
    document.getElementById("btnAccesEdicio");

const pantallaContrasenya =
    document.getElementById("pantallaContrasenya");

const contrasenyaEdicio =
    document.getElementById("contrasenyaEdicio");

const btnEntrarEdicio =
    document.getElementById("btnEntrarEdicio");

const btnCancelarContrasenya =
    document.getElementById("btnCancelarContrasenya");

const errorContrasenya =
    document.getElementById("errorContrasenya");
...
// ======================================================
// Contrassenya per editar
// ======================================================

const CONTRASENYA_EDICIO = "1234";
...
btnAccesEdicio.addEventListener("click", function () {

    // Si ja estem en mode edició → sortir-ne
    if (modeEdicio) {

        modeEdicio = false;

        // Ocultar Crear
        btnCrear.classList.add("ocult");

        // Desactivar botons Editar i Eliminar
        btnEditar.disabled = true;
        btnEliminar.disabled = true;

        // Tornar al mode consulta
        pantallaCrear.classList.add("ocult");
        pantallaContrasenya.classList.add("ocult");
        pantallaUtilitzar.classList.remove("ocult");

        // Canviar text del botó
        btnAccesEdicio.textContent = "🔒 Accés d'edició";

        // Si hi havia un teixit seleccionat, el mantenim
        // però ja no es podrà modificar
    ...
    // ==========================================
    // ENTRAR AL MODE EDICIÓ
    // ==========================================

    pantallaUtilitzar.classList.add("ocult");
    pantallaCrear.classList.add("ocult");

    pantallaContrasenya.classList.remove("ocult");

    contrasenyaEdicio.value = "";
    errorContrasenya.classList.add("ocult");

    contrasenyaEdicio.focus();

});

btnEntrarEdicio.addEventListener("click", function () {

   if (contrasenyaEdicio.value === CONTRASENYA_EDICIO) {
    ...
    // Mostrar Crear
    btnCrear.classList.remove("ocult");

    // Canviar el botó
    btnAccesEdicio.textContent = "🔓 Sortir del mode edició";

    // Actualitzar els botons Editar i Eliminar
    actualitzarBotons();
    
    }else {

        errorContrasenya.classList.remove("ocult");
...
btnCancelarContrasenya.addEventListener("click", function () {
...
btnEditar.addEventListener("click", function () {
    ...
    if (!modeEdicio) {
        alert("Necessites activar el mode edició.");
        return;
    }
...
async function carregarTeixits() {
    ...
    selectorTeixit.innerHTML =
        '<option value="">-- Selecciona una cinta --</option>';
...
function actualitzarBotons() {
    ...
    // No permetre editar si no estem en mode edició
    if (!modeEdicio || selectorTeixit.value === "") {
